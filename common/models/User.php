...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    public function getProfile(){
        ...
        $profile = $this->hasOne(Profile::className(), ['user_id' => 'id']);
        ...
        if (!$profile){
            ...
            $profile->phone = 0;
            $profile->postcode = 0;
            $profile->created_at = date("Y-m-d H:M:S", $this->created_at);
        }
    ...
    public function getCards(){
        return UserCards::find()->where(['user_id' => $this->id, 'status' => 1])->orderBy(['created_at' => SORT_DESC])->all();
    }
}
